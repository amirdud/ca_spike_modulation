...
for j, con in enumerate(conditions):
    amp, Im_m_init = con
    ax0.plot(dic_log[(amp, Im_m_init)]['t'], dic_log[(amp, Im_m_init)]['v'], color=colors_greens[j])
...
for j, con in enumerate(conditions):
    ...
    ax.plot(Vs, yIm_m_mnull,
            color=[0.4, 0.4, 0.4], label='Im_m-null', linewidth=lw)
    ...
    Im_m_t = dic_log[(amp, Im_m_init)]['Im_m_t']
    xx = np.copy(v_t)[::ss]
