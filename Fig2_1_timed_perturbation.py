...
evenly_spaced_interval = np.linspace(0, 1, len(imaxs))
...
for i, imax in enumerate(imaxs):
    for tau0 in tau0s:
        for tau1 in tau1s:
            for onset in onsets:
                if imax == 0:
                    plt.plot(t_shifted_np, dic_log[(imax, tau0, tau1, onset)]['nexus_v'], color='k')
                else:
                    plt.plot(t_shifted_np, dic_log[(imax, tau0, tau1, onset)]['nexus_v'], color=colors[i])
...
for i,imax in enumerate(imaxs):
    for tau0 in tau0s:
        for tau1 in tau1s:
            for onset in onsets:
                area = props[(imax,tau0,tau1,onset)]['area']
...
for i,imax in enumerate(imaxs):
    for tau0 in tau0s:
        for tau1 in tau1s:
            for onset in onsets:
                plt.plot(imax, list_dur[i], 'o', color=colors[i],markersize=8)
